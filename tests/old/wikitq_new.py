        ...
        try:
            ...
            for (i, query), response in zip(current_prompts, responses):
                ...
                if "```python" in output:
                    ...
                    # Extract only the part up to the end of the code block
                    code_pattern = r"```python[\s\S]*?```"
                    match = re.search(code_pattern, full_query)
                    if match:
                        # Include text before and up to the end of the code block
                        end_pos = match.end()
                        truncated_query = full_query[:end_pos]
                        remain_prompts.append((i, truncated_query))
                    else:
            ...
            if remain_codes:
                ...
                for k in range(len(remain_prompts)):
                    ...
                    if model_responses[i].count("```python")>=3:
                        message =  "\nYou have exceeded the maximum number of Python code usage attempts. Please think step-by-step and give me the final answer directly. You are not allowed to use ```python. Do NOT use ```python again.\n\n"
                        query += message
...
def main():
    ...
    try:
        with open(os.path.join(base_path, "tests/few_shots/table_r1_old.md"), 'r', encoding='utf-8') as f:

...
def extract_answer_from_response(model_answer: str) -> str:
    ...
    match = re.search(answer_pattern, model_answer, re.DOTALL)
    if match:
    ...
    for line in reversed(lines):
        if line.strip() and not line.lower().startswith(('thus', 'therefore', 'so', 'hence')):
            # Try to extract numeric expression from this line
            number_match = re.search(r'([$¥€£]?\s*[-+]?[0-9,]*\.?[0-9]+\s*(?:million|billion|thousand|M|B|K)?%?)', line)
            if number_match:
...
def check_answer_correctness(model_answer: str, expected_answer: str) -> Tuple[bool, str]:
    ...
    if model_unit == '$' or model_unit == 'million' or model_unit == 'billion' or expected_unit == '$' or expected_unit == 'million' or expected_unit == 'billion':
...
#     # Check if values are exactly equal - important for integers
#     if model_value == expected_value:
#         return True, extracted_answer
        
#     # Special handling for integers - they must be exactly equal if >100
#     if (model_unit == '' and expected_unit == '' and 
#         model_value > 100 and expected_value > 100 and
#         model_value.is_integer() and expected_value.is_integer()):
#         # For integers >100, require exact match
#         return model_value == expected_value, extracted_answer
    
#     # For small differences in non-integer values, still consider them equal
#     if abs(model_value - expected_value) < 0.01:
#         # For very close values, consider correct
#         return True, extracted_answer
...
#     # Handle percentages more strictly - fix for 16.93% vs 16.82%
#     if model_is_percent and expected_is_percent:
#         # For percentages, require much more precision - no more than 0.05 percentage points difference
#         return abs(model_value - expected_value) < 0.05, extracted_answer
    
#     # Handle monetary amounts
#     if model_unit == '$' or model_unit == 'million' or model_unit == 'billion' or expected_unit == '$' or expected_unit == 'million' or expected_unit == 'billion':
#         # Compare after converting to the same unit
#         # More permissive with currency values
#         if abs(model_value - expected_value) / max(abs(expected_value), 1) < 0.03:
#             return True, extracted_answer
#     else:
#         # General number comparison
#         # If they appear to be integers and both > 100, require exact match
#         if (model_value > 100 and expected_value > 100 and 
#             abs(round(model_value) - model_value) < 0.001 and 
#             abs(round(expected_value) - expected_value) < 0.001):
#             return int(round(model_value)) == int(round(expected_value)), extracted_answer
#         else:
#             # For other numbers, allow small error margin
#             relative_error = abs(model_value - expected_value) / max(abs(expected_value), 1)
#             if relative_error < 0.02:  # Reduced from 5% to 2% error margin
#                 return True, extracted_answer
...
def evaluate_finqa(results_dict: Dict[str, Dict]) -> Tuple[float, Dict[str, Any]]:
    ...
    for qid, item in tqdm(results_dict.items(), desc="Evaluating FinQA results"):
        ...
        is_correct, extracted_answer = check_answer_correctness(model_answer, gold_answer)
...
def save_results(accuracy: float, details: List[Dict], output_file: str, results_file: str):
    """
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Evaluate model answers for FinQA task')
    ...
    print(f"Correct answers: {sum(1 for d in details if d.get('is_correct', False))}/{len(details)}")
    print("=" * 50)

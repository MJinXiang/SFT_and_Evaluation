...
def save_results(acc_score, details, type_acc_scores, output_file: str, results_file: str):
    """
    ...
    results = {
        ...
        "type_accuracy": {qtype: score * 100 for qtype, score in type_acc_scores.items() if score > 0},
...
def main():
    ...
    print(f"Correct Answers: {sum(1 for score in details.values() if score > 0)}/{len(details)}")
    ...
    for qtype, score in type_acc_scores.items():
        if score > 0 and qtype != "unknown":
            ...
            count = len([item for item in results_dict.values() if item.get("type") == qtype])
            if count > 0:
                print(f"{qtype} Accuracy: {score * 100:.2f}% ({int(score * count)}/{count})")
    print("--------------------------------------")
    
    # Save detailed results
    save_results(acc_score, details, type_acc_scores, args.output_file, args.results_file)

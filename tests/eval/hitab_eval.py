...
def match_values(predicted, actual):
    ...
    if len(normalized_predicted.split()) > 0 and is_number(normalized_predicted.split()[0]) and len(normalized_predicted.split()) > 1:
    ...
    if len(actual) > 1:
        ...
        for expected_val in normalized_actuals:
            ...
            found = (expected_val in normalized_predicted or 
                    (is_number(expected_val) and any(match_numeric(expected_val, part) 
...
def save_results(exact_accuracy: float, details: List[Dict], output_file: str):
    ...
    """
    results = {
        "exact_accuracy": exact_accuracy * 100,
        "details": details
    }

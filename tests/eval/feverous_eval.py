...
def evaluate_feverous_results(input_file, output_file=None):
    ...
    if output_file is None:
        ...
        output_file = os.path.join(dirname, f"eval_{basename.split('.')[0]}_{timestamp}.json")
    
    try:
        with open(input_file, 'r', encoding='utf-8') as f:
            data = json.load(f)
    except json.JSONDecodeError:
        print(f"Error: Could not parse input file {input_file}. Please ensure it is valid JSON.")
        return None
    ...
    if "results" in data:
        ...
        print(f"Error: Unexpected data format. Expected a list or an object with 'results' field.")
        return None
    ...
    # Calculate accuracy
    valid_samples = total_samples - stats["samples_with_missing_truth"] - stats["samples_with_missing_prediction"]
    if valid_samples > 0:
    ...
    print(f"Samples with missing model prediction: {stats['samples_with_missing_prediction']}")
    ...
    print("\n===== Confusion Matrix =====")
    print("                   | Predicted SUPPORTS | Predicted REFUTES | Predicted NOT ENOUGH INFO")
    print("-" * 80)
    print(f"True SUPPORTS      | {confusion_matrix['true_supports']['pred_supports']:<19} | {confusion_matrix['true_supports']['pred_refutes']:<18} | {confusion_matrix['true_supports']['pred_nei']}")
    print(f"True REFUTES       | {confusion_matrix['true_refutes']['pred_supports']:<19} | {confusion_matrix['true_refutes']['pred_refutes']:<18} | {confusion_matrix['true_refutes']['pred_nei']}")
    print(f"True NOT ENOUGH INFO | {confusion_matrix['true_nei']['pred_supports']:<19} | {confusion_matrix['true_nei']['pred_refutes']:<18} | {confusion_matrix['true_nei']['pred_nei']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="FEVEROUS prediction results evaluation")

...
def normalize_numeric_answer(answer):
    if isinstance(answer, list):
        return [str(item) if isinstance(item, (int, float)) else item for item in answer]
...
def evaluate(results_dict: Dict[str, Dict]) -> Tuple[float, float]:
    ...
    for qid, item in tqdm(results_dict.items(), desc="Evaluating"):
        ...
        try:
            em_and_f1(ground_truth=ground_truth, prediction=model_answer, pred_scale=scale)
...
def save_results(exact_match: float, f1_score: float, details: List[Dict], output_file: str, results_file: str):
    """
    ...
    for detail in details:
        ...
        if uid in original_data_map:
            ...
            detail_copy["full_response"] = original_item.get("full_response", "")
...
def main():
    ...
    parser.add_argument('--results_file', type=str, default='/netcache/mengjinxiang/Project/LLaMA-Factory-main/results/tatqa/tatqa_sft_grpo_results.json',   #tatqa_sft_filter_results.json
